...
export default function CartScreen() {
    ...
    const updateCartHandler = async (item, quantity) => {
        ...
        if (data.countInStock < quantity) {
            window.alert("Sorry. We don't have that many.");
            return;
        }
                    ...
                    <Card>
                        <Card.Body>
                            <ListGroup variant="flush">
                                <ListGroup.Item>
                                    <h3>
                                        Subtotal ({cartItems.reduce((accumulated, currentItem) => accumulated + currentItem.quantity, 0)}{" "}
                                        items) : $
                                        {cartItems.reduce((accumulated, currentItem) => round2(accumulated + currentItem.price * currentItem.quantity), 0)}
                                    </h3>
                                </ListGroup.Item>
                                <ListGroup.Item>
                                    <div className="d-grid">
                                        <Button type="button" variant="primary" onClick={checkoutHandler} disabled={cartItems.lenght === 0}>
                                            Proceed to Checkout
                                        </Button>
                                    </div>
                                </ListGroup.Item>
